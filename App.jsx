...
const API_BASE_URL = 'https://etl-backend-bj5g.onrender.com';
...
export default function App() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      toast.success('Conversion initiated securely.');
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (statusRes.data.status === 'error') {
            ...
            toast.error('Backend conversion failed. Check server logs.');
          }
          ...
          clearInterval(interval); setIsConverting(false);
          toast.error('Lost connection to the backend server.');
        }
      }, 1000);
    } catch (err) {
      setIsConverting(false); toast.error('Secure conversion failed to start.');
    }
  };
    ...
    <div className={`min-h-screen transition-colors duration-500 ${darkMode ? 'bg-cyber-dark text-white' : 'bg-gray-50 text-gray-900'} cyber-bg relative overflow-hidden flex flex-col`}>
      ...
      <main className="flex-grow container mx-auto px-4 py-12 flex flex-col items-center z-10 max-w-4xl">
        <div className="text-center mb-10">
          ...
          <p className={`text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Secure, client-side encrypted upload. No external tools required.</p>
        </div>
            ...
            <AnimatePresence>
              {file && !downloadUrl && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-6">
                  ...
                  {isConverting && (
                    <div className="mt-4">
                      <div className="flex justify-between text-sm mb-1"><span className="text-cyber-blue">Analyzing Packets...</span><span>{progress}%</span></div>
              ...
              {downloadUrl && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`mt-6 p-6 rounded-xl border border-green-500/30 ${darkMode ? 'bg-green-500/10' : 'bg-green-50'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3"><CheckCircle className="text-green-500 w-8 h-8" /><div><h4 className="font-bold text-lg text-green-500">Conversion Complete</h4></div></div>
        ...
        {history.length > 0 && (
          <div className="w-full mt-12">
            <h3 className="text-xl font-semibold mb-4 flex items-center gap-2"><Clock className="text-cyber-purple" /> Recent Conversions</h3>
                ...
                <tbody>
                  {history.map((item, idx) => (
                    <tr key={idx} className={`border-t ${darkMode ? 'border-white/10' : 'border-gray-200'}`}>
                      ...
                      <td className="p-4 text-right"><a href={item.url} download className="text-cyber-blue">Download</a></td>
                    </tr>
                  ))}
                </tbody>
              </table>
